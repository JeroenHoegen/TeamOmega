			...
			$('#removeReparatie').click(function() {
				if(confirm('Weet u zeker dat u deze reparatie wil verwijderen?')) {
					...
					$.ajax({
						...
						type: 'post',
						data: 'action=removeReparatie&token=<?php echo $_SESSION['token']; ?>&id=<?php echo filterData($reparatieData[0]['id']); ?>',
						dataType: 'json',
						success: function(response) {
							if(response.success) {					
			...
			$('#statusupdates').on('click', '.removeStatus', function() {
				...
				if(confirm('Weet u zeker dat u deze status wil verwijderen?')) {
					...
					$.ajax({
						url: 'resources/remove.ajax.php',
						type: 'post',
						data: 'action=removeStatus&token=<?php echo $_SESSION['token']; ?>&id='+statusId,
						dataType: 'json',
						success: function(response) {
							if(response.success) {		
								$('tr#'+statusId).remove();
